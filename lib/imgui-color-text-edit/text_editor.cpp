...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Z)))
			...
			EnterCharacter('\n', false);
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_KeypadEnter)))
			EnterCharacter('\n', false);
...
{
	mLines.clear();
	mLines.emplace_back(Line());  // Start with one line
	...
	// Check if the last line is empty (was created due to a trailing newline)
	if (mLines.size() > 1 && mLines.back().empty())
	{
		mLines.pop_back();  // Remove the last empty line if added unnecessarily
	}
...
inline uint32_t from_picker(uint32_t rgbValue) {
	// Extract the RGB components from the input
	uint8_t red = (rgbValue >> 16) & 0xFF;
	uint8_t green = (rgbValue >> 8) & 0xFF;
	uint8_t blue = rgbValue & 0xFF;

	// Rearrange to match ImGui's format (ARGB with expected byte order)
	uint32_t imguiColor = (0xFF << 24) | (blue << 16) | (green << 8) | red;
...
const TextEditor::Palette& TextEditor::GetDarkPalette()
{
	const static Palette p = { {
			0xffeaeaea,	// Default (lighter gray)
			0xffd69c56,	// Keyword (unchanged, orange)
			from_picker(0xd0ebae),	// Number (vibrant green)
			0xffce9178,	// String (bright peach)
			0xffd19a66, // Char literal (orange-brown)
			0xffffffff, // Punctuation (white for clarity)
			0xff56b6c2,	// Preprocessor (cyan)
			0xffeaeaea, // Identifier (same as default, light gray)
			from_picker(0xfbffc5), // Known identifier
			0xffc678dd, // Preproc identifier (magenta)
			from_picker(0x79c7f0), // Convention Variables
			from_picker(0x519c3c), // Comment (single line, muted green)
			0xff7f9f7f, // Comment (multi line, muted green)
			0xff1e1e1e, // Background (dark gray)
			0xffffffff, // Cursor (white)
			0x804080ff, // Selection (semi-transparent blue)
			0xffe06c75, // ErrorMarker (bright red)
			0xffbe5046, // Breakpoint (brick red)
			0xff858585, // Line number (dark gray)
			0x20202020, // Current line fill (subtle highlight)
			0x20404040, // Current line fill (inactive)
			0x20808080, // Current line edge (subtle gray)
		} };
	return p;
}
...
const TextEditor::Palette& TextEditor::GetLightPalette()
{
	const static Palette p = { {
			...
			0xffff0c06,	// Keyword	
			0xff098658,	// Number (dark green)
			0xffa31515,	// String (red)
			0xffa31515, // Char literal (same as string)
			0xff000000, // Punctuation (black)
			0xff000080,	// Preprocessor (dark blue)
			0xff000000, // Identifier (black)
			0xff267f99, // Known identifier (teal)
			0xffaf00db, // Preproc identifier (purple)
			0xff986801, // Convention (orange)
			0xff008000, // Comment (single line, green)
			0xff008000, // Comment (multi line, green)
			0xffffffff, // Background (white)
			0xff000000, // Cursor (black)
			0x80600000, // Selection (semi-transparent orange)
			0xffe51400, // ErrorMarker (red)
			0xffd16969, // Breakpoint (soft red)
			0xff6a6a6a, // Line number (gray)
			0x40d3d3d3, // Current line fill (light gray)
			0x40c0c0c0, // Current line fill (inactive)
			0x40a0a0a0, // Current line edge (subtle gray)
		} };
	return p;
}
			...
			else
			{
				...
				if (token_color == PaletteIndex::Identifier)
				{
					...
					if (!line[first - bufferBegin].mPreprocessor)
					{
						...
						else if (mLanguageDefinition.mConvention.count(id) != 0)
							token_color = PaletteIndex::Convention;
...
static bool TokenizeCStyleNumber(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	...
	// Parse digits before the decimal point (integer part)
	while (p < in_end && (*p >= '0' && *p <= '9')) {
		has_digits = true;
		p++;
	}

	// Check for decimal point (floating-point numbers)
	bool is_float = false;
	if (p < in_end && *p == '.') {
		is_float = true;
		p++; // Move past the decimal point
	...
	// If no digits were found, return false (invalid number)
	if (!has_digits)
		return false;

	// Check for exponent part (e.g., 10e+4)
	if (p < in_end && (*p == 'e' || *p == 'E')) {
		is_float = true;
		p++; // Move past 'e' or 'E'
		...
		// Parse digits in the exponent part
		while (p < in_end && (*p >= '0' && *p <= '9')) {
			has_exponent_digits = true;
			p++;
		}

		// If no digits in the exponent part, it's invalid
		if (!has_exponent_digits)
			return false;
	}

	// Optional 'f' or 'F' suffix for single-precision floating-point numbers (e.g., 1.0f)
	if (is_float && p < in_end && (*p == 'f' || *p == 'F')) {
		p++;
	}

	// Optional integer size suffixes (e.g., 'u', 'U', 'l', 'L' for unsigned or long integers)
	while (p < in_end && (*p == 'u' || *p == 'U' || *p == 'l' || *p == 'L')) {
		p++;
	}

	// Set the output pointers to mark the valid token range
	out_begin = in_begin;
	out_end = p;

	// Return true since a valid number was found
	return true;
}
...
bool TokenizeQualifiedIdentifier(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	out_begin = in_begin;

	while (in_begin < in_end && (isalnum(*in_begin) || *in_begin == '_' || *in_begin == ':'))
		in_begin++;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::CPlusPlus()
{
	...
	if (!inited)
	{
		...
		static const char* const identifiers[] = {
			"abort", "abs", "acos", "acosh", "asin", "asinh", "atan", "atan2", "atanh",
			"atexit", "atof", "atoi", "atol", "cbrt", "ceil", "clock", "conj", "copysign",
			"cos", "cosh", "ctime", "div", "erf", "erfc", "exit", "exp",
			"exp2", "expm1", "fabs", "fdim", "floor", "fma", "fmax", "fmin", "fmod",
			"frexp", "getchar", "getenv", "hypot", "ilogb", "imag", "isalnum", "isalpha",
			"isdigit", "isgraph", "ispunct", "isspace", "isupper", "kbhit", "ldexp",
			"lgamma", "llrint", "llround", "log", "log10", "log1p", "log2", "logb",
			"lrint", "lround", "map", "max", "memcmp", "min", "modf", "nan",
			"nearbyint", "nextafter", "nexttoward", "pow", "printf", "putchar", "putenv",
			"puts", "rand", "real", "remainder", "remove", "remquo", "rename", "rint",
			"round", "scalbln", "scalbn", "set", "sin", "sinh", "snprintf", "sprintf",
			"sqrt", "srand", "std", "strcat", "strcmp", "strerror", "string", "tan",
			"tanh", "tgamma", "time", "tolower", "toupper", "trunc", "unordered_map",
			"unordered_set", "vector", "clamp"
		};


		static const char* const constants[] = {
			"e", "egamma", "inv_pi", "inv_sqrt3", "inv_sqrtpi", "ln10", "ln2", "log10e",
			"log2e", "phi", "pi", "sqrt2", "sqrt3"
		};

		static const char* const convention[] = {
			"in", "out", "mem", "timestep",
		};
		...
		for (auto& k : convention)
		{
			Identifier id;
			id.mDeclaration = "gctrl convention";
			langDef.mConvention.insert(std::make_pair(std::string(k), id));
		}
		...
		langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool
			{
				...
				else if (TokenizeQualifiedIdentifier(in_begin, in_end, out_begin, out_end))
